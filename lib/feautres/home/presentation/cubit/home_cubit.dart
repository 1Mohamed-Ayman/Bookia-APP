...
import 'package:bookia/feautres/wishlist/data/repo/wishlist_repo.dart';
...
class HomeCubit extends Cubit<HomeState> {
  ...
  getInitData() async {
    ...
    // results [SliderResponse? , BestSellerResponse?]
  ...
  addRemoveWishlist({required int productId}) async {
    ...
    if (checkIfWishlist(productId)) {
      ...
      if (res != null) {
        emit(HomeErrorState(message: 'Removed from wishlist'));
      } else {
        emit(HomeSuccessState());
      }
    } else {
